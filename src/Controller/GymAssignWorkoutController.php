...
class GymAssignWorkoutController extends AppController
{
	...
	public function assignWorkout()
    {
		...
		{
			$members = $this->GymAssignWorkout->GymMember->find("list",["keyField"=>"id","valueField"=>"name"])->where(["id"=>$session["id"]]);
			$members = $members->select(["id","name"=>$members->func()->concat(["first_name"=>"literal"," ","last_name"=>"literal"])])->hydrate(false)->toArray();
		}
		else{
			$members = $this->GymAssignWorkout->GymMember->find("list",["keyField"=>"id","valueField"=>"name"])->where(["role_name"=>"member","member_type"=>"Member"]);
		...
		foreach($phpobj as $key => $val)
		{
			...
			foreach($val as $key=>$key_val)
			{
				...
				if($key == "activity")
					...
					{
						//if there is no initialization set these value to 0
						$val2['activity']['reps'] = ($val2['activity']['reps'] == '') ? 0 : $val2['activity']['reps'];
						$val2['activity']['sets'] = ($val2['activity']['sets'] == '') ? 0 : $val2['activity']['sets'];
						$val2['activity']['kg'] = ($val2['activity']['kg'] == '') ? 0 : $val2['activity']['kg'];
						$val2['activity']['time'] = ($val2['activity']['time'] == '') ? 0 : $val2['activity']['time'];
	 ...
	 public function viewWorkouts($id)
    {
		...
		if($this->request->is("post"))
		{
			...
			if(!empty($this->request->data["activity_list"]) && $this->GymAssignWorkout->save($row))
			{
				$save = true;
				$workout_id = $row->id;
			}
	...
	public function isAuthorized($user)
	{
		...
		$members_actions = ["memberAssigedWorkout","printWorkout", "assignWorkout"];
