	...
	public function attendance()
    {
		...
		if($this->request->is("post") && isset($this->request->data["attendence"]))
		{
			...
			if($session["role_name"] == "staff_member")
			{
				...
				$data = $this->GymAttendance->GymMember->find("all")
										->contain(["GymMember"])
										->where([
											"GymMember.member_type"=>"Member",
											"GymMember.membership_status"=>"Continue",
											//"GymMemberClass.assign_class" => $class_id,
											"GymMember.assign_staff_mem"=>$session["id"],
											"GymMember.membership_valid_from <= " => $att_date,
											"GymMember.membership_valid_to >= "=> $att_date
				...
				/**	0 => ['id' => (int) 13, 'member_id' => (int) 14072, 'assign_class' => (int) 106,'gym_member' => [
				*/
				$data = $this->GymAttendance->GymMember->find("all")
										->where([
