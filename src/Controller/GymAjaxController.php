...
Class GymAjaxController extends AppController
{
	...
	public function viewGroupMember($gid)
	{
		if($this->request->is("ajax"))
		{
			...
			$data = $grp_tbl->find("all")->where(["role_name"=>"member"])->select(["first_name","last_name","image"])->hydrate(false)->toArray();
		?>
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			...
			<div class="modal-body">
				<div class="panel panel-white form-horizontal">
				  ...
				  <div class="form-group">
					...
					<div class="col-sm-9"> <?=$row["start_date"]->i18nFormat('dd/MM/yyyy')?> <?php //echo date($this->GYMFunction->getSettings("date_format"),strtotime($row["start_date"])); ?> </div>
				  </div>
				  <div class="form-group">
					<label for="notice_title" class="col-sm-3">
					<?php echo __("End Date ");?>   : </label>
					<div class="col-sm-9"> <?=$row["end_date"]->i18nFormat('dd/MM/yyyy')?><?php //echo date($this->GYMFunction->getSettings("date_format"),strtotime($row["end_date"])); ?> </div>
				  </div>
				</div>
			</div>
			<div class="modal-footer">
