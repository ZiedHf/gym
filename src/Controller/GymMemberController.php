...
Class GymMemberController extends AppController
{
	...
	public function addMember()
	{
		...
		if($this->request->is("post"))
		{
			...
			if($ext != 0)
			{
				...
				$this->request->data["birth_date"] = str_replace('/', '-', $this->request->data["birth_date"]);
				$this->request->data['birth_date'] = date("Y-m-d",strtotime($this->request->data['birth_date']));
				$this->request->data["inquiry_date"] = str_replace('/', '-', $this->request->data["inquiry_date"]);
				$this->request->data['inquiry_date'] = date("Y-m-d",strtotime($this->request->data['inquiry_date']));
				$this->request->data["trial_end_date"] = str_replace('/', '-', $this->request->data["trial_end_date"]);
				...
				{
					$this->request->data["membership_valid_from"] = str_replace('/', '-', $this->request->data["membership_valid_from"]);
				...
				{
					$this->request->data["membership_valid_to"] = str_replace('/', '-', $this->request->data["membership_valid_to"]);
					$this->request->data['membership_valid_to'] = date("Y-m-d",strtotime($this->request->data['membership_valid_to']));
				}
				$this->request->data["first_pay_date"] = str_replace('/', '-', $this->request->data["first_pay_date"]);
	...
	public function editMember($id)
	{
		...
		{
			//$classes = $this->GymMember->ClassSchedule->find("list",["keyField"=>"id","valueField"=>"class_name"])->toArray();
			$classes = $this->GymMember->ClassSchedule->find("list",["keyField"=>"id","valueField"=>"class_name"])->where(["id IN"=>$membership_classes])->toArray();
		}else{
			$classes = array();
		}
	...
	public function viewAttendance()
	{
		...
		if($this->request->is("post"))
		{
			...
			$this->request->data["sdate"] = str_replace('/', '-', $this->request->data["sdate"]);
			$s_date = date("Y-m-d",strtotime($this->request->data["sdate"]));
			$this->request->data["edate"] = str_replace('/', '-', $this->request->data["edate"]);
