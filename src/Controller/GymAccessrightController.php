...
class GymAccessrightController extends AppController
{
	public function accessRight()
    {
		...
		if($this->request->is("post"))
		{
			...
			$access_right['map'] =array('menu_icone'=>'map.png' ,'menu_title'=>'Map',
			'controller'=>"Dashboard",
			'action'=>'map',
			'member' => isset($request['member_map'])?$request['member_map']:0,
			'staff_member' =>isset($request['staff_member_map'])?$request['staff_member_map']:0,
			'accountant' =>isset($request['accountant_map'])?$request['accountant_map']:0,
			'page_link'=>$site_path.'/Dashboard/map');
			// var_dump($this);die;
