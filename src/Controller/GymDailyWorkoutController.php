...
class GymDailyWorkoutController extends AppController
{
	public function initialize()
	{
		...
		//$this->MEASURMENT_ARRAY = array("Height" =>__("Height"),"Weight"=>__("Weight"),"Chest"=>__("Chest"),"Waist"=>__("Waist"),"Thigh"=>__("Thigh"),"Arms"=>__("Arms"),"Fat"=>__("Fat"));
	}
	...
	public function workoutList()
	{
		...
		else if($session["role_name"] == "member")
		{
			...
			$measurments = $measurment_table->find()->where(["user_id"=>$uid])->hydrate(false)->toArray();
			$this->set("measurments",$measurments);
		}
		$this->set("data",$data);
	}
	...
	public function addWorkout()
    {
		...
		if($this->request->is("post") && !isset($this->request->data["new_data"]) && !isset($this->request->data["edit"]))
		{
			...
			$this->request->data["record_date"] = str_replace('/', '-', $this->request->data["record_date"]);
		...
		else if($this->request->is("post") && isset($this->request->data["new_data"]) && !isset($this->request->data["edit"]))
		{
			...
			if($this->GymDailyWorkout->GymAssignWorkout->save($assign_row))
			{
				...
				foreach($activities as $activity)
				{
					$error = null;
					$data = array();
					$post["record_date"] = str_replace('/', '-', $post["record_date"]);
	...
	{
		//$this->set("MEASURMENT_ARRAY", $this->MEASURMENT_ARRAY);
		...
		if($this->request->is("post"))
		{
			...
			if($ext != 0)
			{
				...
				$this->request->data["created_by"]= $session["id"];
				$this->request->data["created_date"]= date("Y-m-d");
				$this->request->data["result_date"] = str_replace('/', '-', $this->request->data["result_date"]);
