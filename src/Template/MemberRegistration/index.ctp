	...
	<div class="col-md-12 box box-default">
		...
		<?php
			...
			<div class='form-group'>
				<label class="control-label col-md-2" for="gym"><?=__("Gym")?></label>
				<div class="col-md-6">
					<select class="form-control" name="gym" required>
			      <option value="" selected>Please choose the gym</option>
			      <?php
			        foreach ($gyms as $gym => $data) {
			      ?>
			          <option value="<?=$data['db']?>"><?=$gym?></option>
			      <?php
			        }
			      ?>
			    </select>
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"birth_date","class"=>"form-control dob validate[required] datepick","value"=>(($edit)?date("d-m-Y",strtotime($data['birth_date'])):'')]);
			echo "</div>";
			echo "</div>";
			...
			*/
			...
			echo '<div class="col-md-2">';
			echo $this->Form->input("",["label"=>false,"name"=>"membership_valid_from","class"=>"form-control validate[required] mem_valid_from","value"=>(($edit && $data['membership_valid_from']!="")?date("d-m-Y",strtotime($data['membership_valid_from'])):'')]);
			echo "</div>";
			echo '<div class="col-md-1 no-padding text-center">';
			// echo "To";
			echo "</div>";
			echo '<div class="col-md-2">';
			echo $this->Form->input("",["type"=>"hidden","label"=>false,"name"=>"membership_valid_to","class"=>"form-control validate[required] valid_to","value"=>(($edit && $data['membership_valid_to']!="")?date("d-m-Y",strtotime($data['membership_valid_to'])):''),"readonly"=>true]);
			echo "</div>";
			echo "</div>";
			...
			echo "<a href='".$this->request->base ."/Users/login' class='btn btn-success'>".__('Go Back')."</a>";
			echo '</div>';
			echo '</div>';
			echo $this->Form->end();
		?>
